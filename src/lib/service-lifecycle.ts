...
export async function executeServiceAction(params: ServiceActionParams): Promise<ServiceActionResult> {
  ...
  if (action === 'install') {
    const result = await ensureServiceBootstrapped(adapter, agentName, home, label, uid, apiServerPort);
  ...
  if (action === 'start') {
    const bootstrapResult = await ensureServiceBootstrapped(adapter, agentName, home, label, uid, apiServerPort);
    if (bootstrapResult.code !== 0) {
      return { action: 'start', ...bootstrapResult, running: false, pid: null, timedOut: false, manager: adapter.type, failed: true };
    }

    const startCmd = adapter.buildStartCommand(unitName);
    const result = await runExecFile(startCmd[0], startCmd.slice(1));
    if (result.code !== 0) {
      return { action: 'start', ...result, running: false, pid: null, timedOut: false, manager: adapter.type, failed: true };
    }
  ...
  if (action === 'stop') {
    ...
    if (result.code !== 0) {
      return { action: 'stop', ...result, running: true, pid: null, timedOut: false, manager: adapter.type, failed: true };
    }
  ...
  if (action === 'restart') {
    const bootstrapResult = await ensureServiceBootstrapped(adapter, agentName, home, label, uid, apiServerPort);
    if (bootstrapResult.code !== 0) {
      return { action: 'restart', ...bootstrapResult, running: false, pid: null, timedOut: false, manager: adapter.type, failed: true };
    }

    const restartCmd = adapter.buildRestartCommand(unitName, uid);
    const result = await runExecFile(restartCmd[0], restartCmd.slice(1));
    if (result.code !== 0) {
      return { action: 'restart', ...result, running: false, pid: null, timedOut: false, manager: adapter.type, failed: true };
    }
    const state = await waitForState('running', adapter, uid, unitName);
    if (!state.running) {
      return { action: 'restart', ...result, running: false, pid: null, timedOut: state.timedOut, manager: adapter.type, failed: true };
    }
