  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(typeof data.error === 'string' ? data.error : 'Failed to save MCP configuration');
      }
    } catch {
    ...
    <Card className="flex h-[calc(100dvh-14rem)] flex-col">
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col pt-2">
        <p className="mb-2 text-xs text-muted-foreground">
          ...
          agent&apos;s config. Changes are merged into <code className="rounded bg-muted px-1">config.yaml</code> on
          save.
        </p>
        {loading ? (
