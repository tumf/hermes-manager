  ...
  async function handleApplyTemplate() {
    ...
    try {
      const res = await fetch(
        `/api/mcp-templates?name=${encodeURIComponent(selectedTemplate)}`,
      );
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        toast.error(
          typeof data.error === 'string' ? data.error : t.mcp.failedApplyTemplate,
        );
        return;
      }
  ...
  async function handleSaveAsTemplate() {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        toast.error(
          typeof data.error === 'string' ? data.error : t.mcp.failedSaveTemplate,
        );
        return;
      }
  ...
  async function handleDeleteTemplate() {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        toast.error(
          typeof data.error === 'string' ? data.error : t.mcp.failedDeleteTemplate,
        );
        return;
      }
      ...
      <CardHeader className="shrink-0 gap-3 pb-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={selectedTemplate} onValueChange={setSelectedTemplate}>
            <SelectTrigger
              aria-label={t.mcp.templatesSection}
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t.mcp.saveTemplateDialogDescription}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
