...
- [x] `hosting/ai.hermes.manager.plist` の固定 `WorkingDirectory` / `StandardOutPath` / `StandardErrorPath` を現行 repo 配置へ更新する（verification: manual - `hosting/ai.hermes.manager.plist` を確認し `<repo-root>` と `runtime/logs/*.log` を参照する）
...
- [x] strict validation を通す（verification: manual - `python3 ~/.agents/skills/cflx-proposal/scripts/cflx.py validate fix-hosting-launchd-bootstrap-errors --strict`）
