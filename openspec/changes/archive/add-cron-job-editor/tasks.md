...
- [x] Task 2: Implement the cron job detail/editor UI in `src/components/cron-tab.tsx` (or a colocated extracted component) with editable fields `name`, `schedule`, `prompt`, `skills`, `deliver`, `repeat`, `model`, `provider` plus read-only runtime fields such as `id`, `state`, `enabled`, `created_at`, `next_run_at`, `last_run_at`, `last_status`, and `last_error`. (verification: `src/components/cron-tab.tsx` source contains form controls for editable fields and rendered runtime metadata labels)
...
- [x] Task 5: Update `docs/requirements.md` and `docs/design.md` so the Cron tab behavior explicitly includes existing-job detail viewing and editing. (verification: `npm run lint` passes and `docs/requirements.md` and `docs/design.md` Cron tab sections mention existing-job detail/edit behavior)
