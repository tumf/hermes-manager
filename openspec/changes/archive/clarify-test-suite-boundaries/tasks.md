...
- [x] Run proposal follow-up verification once implemented (verification: `npm run test && npm run typecheck && npm run lint && npm run test:e2e`; note that `test:e2e` currently validates Playwright runner wiring only because no committed specs exist under `tests/e2e`).
...
- [x] Restore a truthful Playwright E2E boundary by either moving maintained browser tests into `tests/e2e/` or updating the checklist/docs to explicitly state that no E2E coverage currently exists.
- [x] Update verification/checklist claims so completed tasks do not assert `npm run test:e2e` coverage when the script passes only because `--pass-with-no-tests` is enabled.
