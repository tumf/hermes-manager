...
Hermes Agent v0.9.0 introduced an official local web dashboard that overlaps with several areas already covered by Hermes Manager, including sessions, logs, cron, env/config editing, and skills browsing.

Without an explicit product policy, Hermes Manager risks drifting into a feature-for-feature clone of the upstream dashboard, creating three problems:

1. Product positioning becomes unclear because users cannot tell when to use the official dashboard versus Hermes Manager.
...
1. Core differentiators to double down on in Hermes Manager:
...
2. Shared capabilities to keep in Hermes Manager only when they directly support multi-agent operations:
...
3. Upstream-led capabilities that Hermes Manager should not try to outgrow into a parallel full dashboard:
...
4. Integration and interoperability principles:
   ...
   - keep Hermes Manager focused on managing fleets of Hermes agents rather than replacing the official dashboard

## Acceptance Criteria

- [ ] A canonical product policy defines Hermes Manager as a multi-agent management layer, not a replacement for the official single-install Hermes dashboard.
...
- Removal of existing chat/log/cron functionality from Hermes Manager
- UI redesign work
