...
The hermes-agents UI MUST remain optimized for operating multiple managed Hermes agents on one host rather than acting as a general-purpose replacement for the official Hermes single-install dashboard.

#### Scenario: operator navigates between managed agents

**Given** multiple managed agents exist under `runtime/agents/`
**When** the operator uses the hermes-agents UI
...
**Given** Hermes upstream provides an official dashboard feature that overlaps with a hermes-agents UI area
**When** a new hermes-agents proposal introduces or expands a similar feature
...
When hermes-agents includes chat, session, log, cron, env, or skills interfaces, those interfaces MUST be scoped to workflows needed to operate a selected managed agent.
...
**And** the product does not require hermes-agents to become the most comprehensive standalone session exploration UI
