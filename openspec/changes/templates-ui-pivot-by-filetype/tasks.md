...
- [x] `templates` 配列を `FILE_NAMES` でグルーピングする変換ロジックを追加
  - 型: `{ file: string; variants: string[] }[]`
  - `variants` はアルファベット順ソート
  - 該当テンプレートが0件のファイル種別は除外
- [x] カード表示を FILE_NAMES 軸に変更
  - カードタイトル: ファイル名（`AGENTS.md` 等）
  - カード内行: テンプレート名（`default` 等）+ 編集・削除ボタン
- [x] 展開/折りたたみの `expandedTemplates` のキーをファイル名に変更
  - 初期値: `new Set(FILE_NAMES)` （すべて展開）
- [x] テンプレート全体削除ボタン（`handleDeleteTemplate`）を UI から除去
