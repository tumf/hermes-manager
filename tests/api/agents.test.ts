...
describe('GET /api/agents', () => {
  ...
  it('returns agents list', async () => {
    mockState.agentRows = [
      { id: 1, name: 'alpha', home: '/agents/alpha', label: 'ai.hermes.gateway.alpha', enabled: false, createdAt: new Date() },
    ];
    const res = await GET();
    expect(res.status).toBe(200);
    const body = await res.json();
...
describe('POST /api/agents', () => {
  ...
  it('creates agent with valid name and returns 201', async () => {
    const created = { id: 1, name: 'alpha_1', home: '/agents/alpha_1', label: 'ai.hermes.gateway.alpha_1', enabled: false, createdAt: new Date() };
  ...
  it('scaffolds filesystem on create', async () => {
    mockState.insertRows = [{ id: 2, name: 'beta', home: '/agents/beta', label: 'ai.hermes.gateway.beta', enabled: false, createdAt: new Date() }];
...
describe('DELETE /api/agents', () => {
  ...
  it('deletes agent and returns ok', async () => {
    mockState.agentRows = [{ id: 1, name: 'bravo', home: '/agents/bravo', label: 'ai.hermes.gateway.bravo', enabled: false, createdAt: new Date() }];
  ...
  it('calls fs.rm with purge=true', async () => {
    mockState.agentRows = [{ id: 1, name: 'charlie', home: '/agents/charlie', label: 'ai.hermes.gateway.charlie', enabled: false, createdAt: new Date() }];

    const req = makeReq('http://localhost/api/agents?name=charlie&purge=true', { method: 'DELETE' });
    await DELETE(req);
    expect(vi.mocked(fs.rm)).toHaveBeenCalledWith('/agents/charlie', { recursive: true, force: true });
  });
  ...
  it('best-effort stops launchd service', async () => {
    ...
    const req = makeReq('http://localhost/api/agents?name=bravo', { method: 'DELETE' });
    await DELETE(req);
    expect(vi.mocked(execFile)).toHaveBeenCalledWith('launchctl', expect.arrayContaining(['unload']), expect.any(Function));
  });
});
...
describe('POST /api/agents/copy', () => {
  ...
  it('copies agent and returns 201', async () => {
    mockState.agentRows = [{ id: 1, name: 'delta', home: '/agents/delta', label: 'ai.hermes.gateway.delta', enabled: false, createdAt: new Date() }];
    mockState.insertRows = [{ id: 2, name: 'echo', home: '/agents/echo', label: 'ai.hermes.gateway.echo', enabled: false, createdAt: new Date() }];
  ...
  it('calls fs.cp recursively', async () => {
    mockState.agentRows = [{ id: 1, name: 'delta', home: '/agents/delta', label: 'ai.hermes.gateway.delta', enabled: false, createdAt: new Date() }];
    mockState.insertRows = [{ id: 2, name: 'foxtrot', home: '/agents/foxtrot', label: 'ai.hermes.gateway.foxtrot', enabled: false, createdAt: new Date() }];
    ...
    await COPY_POST(req);
    expect(vi.mocked(fs.cp)).toHaveBeenCalledWith('/agents/delta', expect.stringContaining('foxtrot'), { recursive: true });
  });
