...
const ALPHA = { id: 1, name: 'alpha', home: '/agents/alpha', label: 'ai.hermes.gateway.alpha', enabled: false, createdAt: new Date() };
...
describe('DELETE /api/env', () => {
  ...
  it('returns 404 for unknown agent', async () => {
    mockState.agentRows = [];
    const res = await DELETE(makeReq('http://localhost/api/env?agent=ghost&key=FOO', { method: 'DELETE' }));
    expect(res.status).toBe(404);
  });
  ...
  it('removes a key from .env file', async () => {
    ...
    const res = await DELETE(makeReq('http://localhost/api/env?agent=alpha&key=REMOVE_ME', { method: 'DELETE' }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true });
...
describe('GET /api/env/resolved', () => {
  ...
  it('returns merged view with source annotations', async () => {
    ...
    expect(baseUrl).toEqual({ key: 'BASE_URL', value: 'https://override.example.com', source: 'agent-override' });
