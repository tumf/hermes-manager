...
describe('GET /api/mcp-templates', () => {
  ...
  it('lists saved MCP templates', async () => {
    ...
    fs.writeFileSync(
      path.join(tempDir, 'mcp-templates', 'filesystem-default.yaml'),
      'fs: {}\n',
    );
  ...
  it('returns 400 when name is invalid', async () => {
    const res = await GET(
      makeReq('http://localhost/api/mcp-templates?name=../evil'),
    );
...
describe('POST /api/mcp-templates', () => {
  ...
  it('creates a new MCP template', async () => {
    ...
    expect(
      fs.existsSync(path.join(tempDir, 'mcp-templates', 'github-default.yaml')),
    ).toBe(true);
  });
...
describe('DELETE /api/mcp-templates', () => {
  ...
  it('returns 400 when name missing', async () => {
    const res = await DELETE(
      makeReq('http://localhost/api/mcp-templates', { method: 'DELETE' }),
    );
    expect(res.status).toBe(400);
  });
