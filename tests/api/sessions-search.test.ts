...
describe('GET /api/agents/[id]/sessions/search', () => {
  ...
  it('returns 404 when agent not found', async () => {
    ...
    const res = await GET(
      makeReq('http://localhost/api/agents/alpha/sessions/search?q=test'),
  ...
  it('returns 400 when query is missing', async () => {
    const res = await GET(
      makeReq('http://localhost/api/agents/alpha/sessions/search'),
      { params: Promise.resolve({ id: 'alpha' }) },
    );
  ...
  it('returns 400 when query is too short', async () => {
    const res = await GET(
      makeReq('http://localhost/api/agents/alpha/sessions/search?q=a'),
      { params: Promise.resolve({ id: 'alpha' }) },
    );
    expect(res.status).toBe(400);
  });
