...
describe('chat search UI', () => {
  ...
  it('shows search results when query is entered', async () => {
    const fetch = createFetchRouter(buildChatFixtureRoutes({ searchResults: SEARCH_RESULTS }));
    global.fetch = fetch as typeof fetch;
  ...
  it('shows no-results message when search returns empty', async () => {
    const fetch = createFetchRouter(buildChatFixtureRoutes({ searchResults: [] }));
    global.fetch = fetch as typeof fetch;
  ...
  it('selects session when search result is clicked', async () => {
    ...
    const resultButtons = screen
      .getAllByRole('button')
      .filter((btn) => btn.textContent?.includes('gateway'));
