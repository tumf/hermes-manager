...
## 2. 用語
- Agent: Hermes gateway を launchd で常駐させる単位。HERMES_HOME は {PROJECT_ROOT}/agents/{name}。
- Global Vars: 全エージェント共通で参照する .env 値。globals/.env に生成され、dotenvx -f で積み上げる。
...
- in scope:
  ...
  - 変数管理: HERMES_HOME/.env CRUD、グローバル変数 CRUD と globals/.env 再生成
...
- FR-5 Globals API: CRUD、書き換え時に globals/.env を再生成
...
- NFR-4 運用性: ログファイルは data/logs および agent logs/ に分離、restic バックアップ対象
...
## 8. 運用要件
- DB: ./data/app.db（better-sqlite3）。drizzle-kit によりマイグレート。
- バックアップ: data/ と agents/ は resticprofile に追加推奨。
- 監視: 最低限 webapp.log の死活、必要に応じてエンドポイント ping。

## 9. 制約/前提
- Node >= 20, macOS launchd 環境、dotenvx インストール済
- HERMES_HOME 構造: agents/{name}/{AGENTS.md, SOUL.md, config.yaml, .env, logs/}
