...
- Template（ファイルシステムベース）
  ...
  - エージェント作成ダイアログでは memory / user / soul / config を個別に選択できる
...
- /runtime/agents/{agentId}/
  ...
  - skills/hermes-manager-subagent-dispatch/
    - SKILL.md（dispatch enabled 時に自動管理）
    - scripts/dispatch-subagent.sh（managed skill に同梱される dispatch helper）
...
- /api/files: GET/PUT（SOUL.md/SOUL.src.md/memories/MEMORY.md/memories/USER.md/config.yaml）
  - YAML 構文チェック（config.yaml。`mcp_servers` を含む Hermes runtime config もこの経路で編集する）
  - 原子書き込み（.tmp→rename）
...
- Layout: サイドバー（/、/globals、/templates、/partials へのナビ）、モバイルはシート/ドロワ、デスクトップは collapsible sticky sidebar
- Agents 一覧: 表示名（`name` 未設定時は `agentId`）、tags バッジ、enabled, 状態バッジ、Memory、起動/停止、追加（ダイアログ + メタデータ入力 + app-managed file のテンプレート選択、ID自動生成）/削除/コピー（Hermes バージョンは一覧に表示しない）。一覧 UI は fleet inventory と lifecycle 操作を最優先にする。一覧本体の描画は `/api/agents` 完了時点で行い、起動状態は `POST /api/launchd/statuses` で非同期 batch hydrate する。status 取得中は "Checking…" 相当の loading 表示、取得失敗 agent は Stopped と断定せず Unknown の fallback バッジを表示する
...
- Agent 詳細: ヘッダーに name/description/tags と Hermes バージョン（未取得時は `--`）を表示、タブ（Metadata/Memory/Config/Env/Skills/Delegation/Cron/Chat/Logs）。agent detail は managed agent の運用・診断ワークフローを支える範囲に留め、単一 install 向け総合 dashboard の feature parity を目指さない
  - MCP サーバ設定は Config タブの `config.yaml` 編集に含めて扱い、`mcp_servers` だけの専用 UI surface は持たない
  ...
  - Cron タブは `/api/cron` で job の CRUD を行う
    ...
    - 既存ジョブ詳細/編集ビュー: ジョブ名クリックまたは行アクションメニューの "View / Edit" から開き、`name`/`schedule`/`prompt`/`skills`/`deliver`/`repeat`/`model`/`provider` を編集して `PUT /api/cron`（`agent` + `id` + 編集対象フィールド）で保存。読み取り専用 runtime metadata（`id`/`state`/`enabled`/`created_at`/`next_run_at`/`last_run_at`/`last_status`/`last_error`）を同じ画面に表示し、API バリデーションエラーは詳細ビュー内に inline 表示してジョブリストへのナビゲーションを発生させない。保存成功時は `/api/cron` GET でジョブリストを再取得する
...
- Templates 管理: テンプレート名（ディレクトリ）単位で対象ファイルを列挙し、個別ファイルを追加/編集/削除する。対象は `SOUL.md` / `memories/MEMORY.md` / `memories/USER.md` / `config.yaml`
