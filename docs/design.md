...
- Template（ファイルシステムベース）
  ...
  - fileName: 'MEMORY.md' | 'USER.md' | 'SOUL.md' | 'config.yaml'（各ファイルは任意サブセット）
...
- /runtime/agents/{agentId}/
  - MEMORY.md, USER.md, SOUL.md, config.yaml, .env, logs/
  - meta.json（任意: name/description/tags）
- /runtime/templates/{templateName}/
  - MEMORY.md, USER.md, SOUL.md, config.yaml（各ファイルは任意サブセット）
  - `default/` は起動時に自動配置
...
- /api/files: GET/PUT（MEMORY.md/USER.md/SOUL.md/config.yaml）
...
- /api/templates: ファイルシステムベース（runtime/templates/ を直接操作）
  ...
  - file: 'MEMORY.md' | 'USER.md' | 'SOUL.md' | 'config.yaml'
...
- Agent 詳細: ヘッダーに name/description/tags の表示のみ、タブ（Metadata/Memory/Config/Env/Skills/Cron/Chat/Logs）
  ...
  - Memory は `MEMORY.md` / `USER.md` / `SOUL.md` を切替ボタンで選択し、常に1ファイルのみ編集表示
...
- 各 FileEditor に "Save as Template" ボタン（Memory タブの MEMORY.md/USER.md/SOUL.md、Config タブの config.yaml）
