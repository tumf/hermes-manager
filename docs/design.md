...
- MCP Template（ファイルシステムベース、fragment 単位）
  - ストレージ: `runtime/mcp-templates/{name}.yaml`
  - name: string（[a-zA-Z0-9_-]+、filesystem traversal 対策あり）
  - 内容: `mcp_servers` として展開される YAML mapping/object（非 object は保存拒否）
  - full-file templates とは別 store として管理し、MCP フラグメントのみを横断再利用可能にする
- Partial
...
### dispatch → `dispatch.json`（legacy fallback: `delegation.json`）

- canonical パス: `runtime/agents/{agentId}/dispatch.json`
- legacy フォールバック: `runtime/agents/{agentId}/delegation.json`（`dispatch.json` 不在時のみ読み取り）
- 形式:
...
- 作成/更新時: `PUT /api/agents/{id}/delegation` で zod バリデーション後に `dispatch.json` へ保存
- cycle 検出: fleet 全体の dispatch graph で DFS を実行し、cycle 形成時は 409 で拒否
- policy 保存時の副作用:
  ...
  - 生成 guidance は dispatch-first wording を使用し、built-in `delegate_task` は別機構として扱い、parent 再 ownership セマンティクスを明記
...
- /runtime/agents/{agentId}/
  ...
  - dispatch.json（任意: allowedAgents/maxHop — managed dispatch policy。不在時は legacy delegation.json をフォールバック読み取り）
  - skills/hermes-manager-subagent-dispatch/（dispatch enabled 時に自動管理）
- /runtime/templates/{templateName}/
  - SOUL.md, memories/MEMORY.md, memories/USER.md, config.yaml（各ファイルは任意サブセット）
  - `default/` は起動時に自動配置
- /runtime/mcp-templates/
  - `{name}.yaml`（再利用可能な `mcp_servers` フラグメント。YAML mapping/object のみ許可）
- /runtime/partials/
...
- /api/launchd/statuses: POST { agents: string[] }
  - fleet UI 向けの batch status endpoint。単体 `/api/launchd` の契約は変更しない補助 API
  - zod で agent 名配列（1件以上・上限 200）を検証し、重複は除去してから並列に status を問い合わせる
  - レスポンス: `{ statuses: Array<{ agent, running, pid, code, manager, error? }> }`
  - agent が存在しない／個別 status 取得が失敗した要素は `running=null` + `error` を返し、batch 全体は 200 で返す
...
- /api/mcp-templates: ファイルシステムベース（runtime/mcp-templates/ を直接操作）
  - GET /api/mcp-templates → `[{ name }]` MCP テンプレート一覧
  - GET /api/mcp-templates?name=... → `{ name, content }` 個別テンプレート取得
  - POST /api/mcp-templates `{ name, content }` → 新規作成（409 on duplicate、422 on invalid/非-object YAML）
  - PUT /api/mcp-templates `{ name, content }` → 既存更新（404 if not found、422 on invalid/非-object YAML）
  - DELETE /api/mcp-templates?name=... → 削除
  - name: MCP テンプレート名（[a-zA-Z0-9_-]+）
  - POST /api/agents に `mcpTemplate?: string` を追加: 指定時は scaffolding 後に template の内容を `config.yaml` の `mcp_servers` へ merge する（他 keys は保持）
...
- Agents 一覧: 表示名（`name` 未設定時は `agentId`）、tags バッジ、enabled, 状態バッジ、Memory、起動/停止、追加（ダイアログ + メタデータ入力 + テンプレート選択、ID自動生成）/削除/コピー（Hermes バージョンは一覧に表示しない）。一覧 UI は fleet inventory と lifecycle 操作を最優先にする。一覧本体の描画は `/api/agents` 完了時点で行い、起動状態は `POST /api/launchd/statuses` で非同期 batch hydrate する。status 取得中は "Checking…" 相当の loading 表示、取得失敗 agent は Stopped と断定せず Unknown の fallback バッジを表示する
  - 一覧上部に metadata tags によるタグ絞り込み UI を配置する。現在ロード済み agent の tags からユニーク集合を導出して active/inactive chip として表示し、選択は複数タグの OR 条件で適用する。タグ未選択時は全 agent を表示し、タグを持つ agent が存在しない場合は filter chrome を非表示にする。絞り込み結果が 0 件のときは 0 件メッセージとフィルタ解除導線を表示する。API 追加は不要で、`/api/agents` の結果に対してクライアント側で filtering する
- Agent 詳細: ヘッダーに name/description/tags と Hermes バージョン（未取得時は `--`）を表示、タブ（Metadata/Memory/Config/MCP/Env/Skills/Delegation/Cron/Chat/Logs）。agent detail は managed agent の運用・診断ワークフローを支える範囲に留め、単一 install 向け総合 dashboard の feature parity を目指さない
  - MCP タブは `/api/agents/{id}/mcp` から `config.yaml` の `mcp_servers` フラグメントを load/save する dedicated YAML editor を提供し、upstream Hermes MCP ガイドへのリンクを表示する。さらに `/api/mcp-templates` の template を一覧表示し、Apply（editor に挿入、保存は既存 Save フロー経由）、Save as template（現在の MCP YAML を名前付きで保存/更新）、Delete template（選択中テンプレートを削除）の操作を提供する
  - Add Agent ダイアログは既存の file template 選択に加えて、optional な MCP template 選択を受け取り、指定時は作成後に `config.yaml` の `mcp_servers` へ merge する
...
- `AppShell` は `h-dvh` + `overflow-hidden` の 2 カラムシェルとし、デスクトップでは左サイドバーを `sticky top-0 h-dvh` で viewport に固定する
- 右側の `main` ペインだけを `overflow-y-auto` とし、ページ本文スクロールとサイドバー位置を分離する
- デスクトップサイドバーは expanded (`w-64`) / collapsed (`w-20`) を切り替え可能にし、collapsed 時は icon-only rail + tooltip + `aria-label` で操作可能性を維持する
- 折りたたみ状態は `localStorage` (`hermes-desktop-sidebar-collapsed`) に保存し、再読込後に復元する
- モバイルでは従来どおり `Sheet` ベースメニューを使い、ルート変更時に自動 close する
